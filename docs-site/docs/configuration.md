...
| `BARTLEBY_PUBLIC_URL` | *(empty)* | Public HTTPS origin where the server is reachable (no trailing path). Setting it enables the embedded OAuth server and becomes the token issuer / resource identifier. Required to connect from claude.ai. |
| `BARTLEBY_OAUTH_PASSWORD` | *(empty)* | Password for the OAuth login/consent page. Required whenever `BARTLEBY_PUBLIC_URL` is set; the server refuses to start otherwise. |
...
store a static bearer token. Set **both** `BARTLEBY_PUBLIC_URL` (your public HTTPS
origin) and `BARTLEBY_OAUTH_PASSWORD` to turn on Bartleby's embedded OAuth 2.1
...
- `BARTLEBY_PUBLIC_URL` is the OAuth **issuer** and resource identifier. It must
...
- `BARTLEBY_OAUTH_PASSWORD` gates the consent page. Dynamic Client Registration
...
- The static `BARTLEBY_AUTH_TOKEN` keeps working everywhere. When OAuth is on,
...
- Registered OAuth clients and issued tokens are stored in
  `<vault>/.bartleby/oauth.db`. Unlike the search index, this database is **not**
...
When `BARTLEBY_PUBLIC_URL` is unset, none of this is active and the server runs
