...
/api/v1/links` endpoint. Bartleby fetches the URL, extracts the article to
...
- Authentication is the same static `BARTLEBY_AUTH_TOKEN` used by the
...
- A reachable Bartleby server. The Shortcut runs on your iPhone, so the URL
  ...
  - **Tailscale / WireGuard**: the phone joins your private network; point the
    Shortcut at the magic-DNS name (`http://bartleby.tailnet-name.ts.net`).
...
- The `BARTLEBY_AUTH_TOKEN` value the server was started with.
...
2. **Text** action — paste your server's base URL:
   ```
   https://bartleby.example.com/api/v1/links
   ```
3. **Text** action — paste your bearer token:
   ```
   <your BARTLEBY_AUTH_TOKEN>
   ```
...
7. **If** the value has **any value**:
   - **Show Notification** → "Bartleby: " + (Dictionary value for key `error`
     → key `message`).
   - **Otherwise**:
     - **Get Dictionary Value** → get value for key `title` from the
       Get-Contents-of-URL output.
     - **Show Notification** → "Saved: " + that value.
8. Rename the Shortcut to **Save to Bartleby**. Optionally pick an icon and
   colour.

That is the whole flow. From Safari, tap the share button on any page, scroll
to **Save to Bartleby**, and the article lands in your vault. Open the web UI
at `/` to confirm.
...
```bash
curl -sX POST https://bartleby.example.com/api/v1/links \
  -H 'Authorization: Bearer <your-token>' \
  -H 'Content-Type: application/json' \
  -d '{"url": "https://en.wikipedia.org/wiki/Bartleby,_the_Scrivener"}' | jq
```
...
returns Bartleby's standard error envelope:
...
- The Shortcut uses the same static `BARTLEBY_AUTH_TOKEN` as the browser
...
- Bartleby does not try to bypass paywalls or run JavaScript. Pages that need
