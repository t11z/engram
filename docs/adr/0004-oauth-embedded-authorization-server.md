...
token. It has no field for a pre-shared bearer token. Bartleby until now
authenticated solely with a single static `BARTLEBY_AUTH_TOKEN` shared by REST and
...
- **Embedded authorization server.** Bartleby issues its own tokens; clients
...
1. **Bartleby is its own issuer** — it mints and verifies its own tokens; clients
   register dynamically (RFC 7591). The user only pastes the `/mcp` URL into
   claude.ai.
2. **A password gate** (`BARTLEBY_OAUTH_PASSWORD`) guards a minimal login/consent
...
3. **Persistent SQLite store** at `<vault>/.bartleby/oauth.db`, separate from the
   rebuildable FTS index, so registered clients and live tokens survive restarts
   and claude.ai stays connected.

OAuth is **opt-in**: it activates only when `BARTLEBY_PUBLIC_URL` is set (that URL
...
static `BARTLEBY_AUTH_TOKEN`, preserving every existing client.
...
- New opt-in config: `BARTLEBY_PUBLIC_URL` (enables OAuth, must be the public
  HTTPS origin) and `BARTLEBY_OAUTH_PASSWORD` (required whenever the public URL is
...
- OAuth must be deployed behind HTTPS with a correct `BARTLEBY_PUBLIC_URL`; the
