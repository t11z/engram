...
async function onSave(): Promise<void> {
  ...
  try {
    ...
    show("Enter a valid server URL (including https://).");
    return;
  }
  const granted = await chrome.permissions.request({ origins: [`${origin}/*`] });
  if (!granted) {
    show("Host permission denied — the extension can't reach that server.");
    return;
  }
...
async function onTest(): Promise<void> {
  ...
  show(ok ? "Connected." : "No response from server.");
}
